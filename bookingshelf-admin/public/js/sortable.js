...
(function sortableModule(factory) {
  ...
  const _autoScroll = _throttle(function(/** Event*/evt, /** Object*/options, /** HTMLElement*/rootEl) {
  ...
  Sortable.prototype = /** @lends Sortable.prototype */ {
    ...
    _onTapStart: function(/** Event|TouchEvent */evt) {
      ...
      const originalTarget = evt.target.shadowRoot && (evt.path && evt.path[0]) || target;
    ...
    _onTouchMove: function(/** TouchEvent*/evt) {
      if (tapEvt) {
        ...
        const dx = (touch.clientX - tapEvt.clientX) + fallbackOffset.x;
        const dy = (touch.clientY - tapEvt.clientY) + fallbackOffset.y;
        const translate3d = evt.touches ? 'translate3d(' + dx + 'px,' + dy + 'px,0)' : 'translate(' + dx + 'px,' + dy + 'px)';
    ...
    _onDragOver: function(/** Event*/evt) {
      ...
      let isMovingBetweenSortable = false;
      ...
      if (activeSortable && !options.disabled &&
        ...
        if ((el.children.length === 0) || (el.children[0] === ghostEl) ||
          ...
          const floating = R_FLOAT.test(lastCSS.cssFloat + lastCSS.display)
							|| (lastParentCSS.display == 'flex' && lastParentCSS['flex-direction'].indexOf('row') === 0);
          const isWide = (target.offsetWidth > dragEl.offsetWidth);
          const isLong = (target.offsetHeight > dragEl.offsetHeight);
          const halfway = (floating ? (evt.clientX - targetRect.left) / width : (evt.clientY - targetRect.top) / height) > 0.5;
    ...
    /**
		 ...
		 */
    sort: function(order) {
      const items = {}; const rootEl = this.el;
  ...
  function _find(ctx, tagName, iterator) {
    if (ctx) {
      const list = ctx.getElementsByTagName(tagName); let i = 0; const n = list.length;
