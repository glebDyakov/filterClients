...
class Index extends Component {
  ...
  componentWillReceiveProps(newProps) {
    ...
    //       : newProps.notification.notification,
  ...
  setTab(tab) {
    ...
    if (tab === 'auto_notification') {
      document.title = this.props.t('Авто уведомления | Онлайн-запись');
    }
    if (tab === 'smsletter') {
      document.title = this.props.t('SMS Рассылка | Онлайн-запись');
    }
    if (tab === 'newsletter') {
      document.title = this.props.t('Email Рассылка | Онлайн-запись');
    }
    if (tab === 'balance') {
      document.title = this.props.t('Баланс Email/SMS | Онлайн-запись');
    }
    if (tab === 'history') {
      document.title = this.props.t('История сообщений | Онлайн-запись');
    }
  ...
  toggleChange(type) {
    ...
    const notify = { ...notifications, [type]: !notifications[type] };
  ...
  handleChangeSMStoggle(type) {
    ...
    const { sms, staff, count_sms } = this.state;
    let receivers_all = 0;
  ...
  handleChange(e) {
    ...
    const notify = { ...notifications, [name]: value };
  ...
  handleChangeSMS(e) {
    ...
    this.setState({ sms: { ...sms, [name]: value } });
  }
  ...
  handleChangeSMS_text(e) {
    ...
    const count_sms = Math.ceil(value.length / letters);
  ...
  getMessageStatus(messageStatus) {
    switch (messageStatus) {
      case 'DELIVERED':
        return {
          text: this.props.t('Доставлено'),
          color: '#34C38F',
        };
      case 'ERROR':
      case 'blocked':
        return {
          text: this.props.t('Заблокировано'),
          color: '#F46A6A',
        };
      case 'NOT_DELIVERED':
      case 'UNSUCCESS':
        return {
          text: this.props.t('Не доставлено'),
          color: '#F46A6A',
        };
      case 'PENDING':
        return {
          text: this.props.t('Доставляется'),
          color: '#F3933A',
        };
      default:
        return {
  ...
  render() {
    const {
      notification, activeTab, sms, notifications, count_sms, email, editorState, receivers,
    ...
    return (
      <div className="emailPage">
        ...
        {this.state.isLoading &&
          <div className="loader loader-email">
          ...
          <div className="header-tabs-container">
            <ul className="nav nav-tabs">
              ...
              <li className="nav-item">
                <a
                  className={
                    'nav-link' + (activeTab === 'newsletter' || activeTab === 'smsletter' ? ' show active' : '')
                  }
                  data-toggle="tab"
                  href="#newsletter"
                  onClick={() => {
              ...
              <li className="nav-item">
                <a className={'nav-link' + (activeTab === 'balance' ? ' show active' : '')}
                  data-toggle="tab" href="#balance" onClick={() => this.setTab('balance')}
                >
                  {notification.balance && notification.balance.smsAmount} sms
                                    / {notification.balance && notification.balance.emailAmount} email
                </a>
              </li>
              <li className="nav-item">
                    ...
                    <div className="row">
                      <div className="col-md-8">
                        <span className="page-warning">
                          {t("Постарайтесь избегать записи клиентов в журнал в позднее время")}, {t("т.к. клиенты получают моментальные sms о записи")}.
                        </span>
                      </div>
                    </div>
                  </div>
              ...
              <div className="tab-content letter-page">
                ...
                <div className={'tab-pane' + (activeTab === 'smsletter' ? ' active' : '')}
                  ...
                  <div className="block-style1">
                    ...
                    <p className="count-letters">
                      {t("Количество смс")}: {count_sms}, {t("количество символов")}: {sms.description.length}
                    </p>
                    ...
                    <button type="submit"
                      className={'button' + ' ' + ((sms.description === '' || (!sms.toClients && !sms.toStaffs)) &&
                        ' disabledField')}
                      data-toggle="modal"
                ...
                <div className={'tab-pane' + (activeTab === 'newsletter' ? ' active' : '')} id="sms_tab1">
                  <div className="block-style1">
                    ...
                    <button type="button"
                      className={'button' + ' ' + ((email.title === '' || email.description === '' ||
                        (!email.toClients && !email.toStaffs)) && ' disabledField')
                      }
                      data-toggle="modal"
                    ...
                    <div style={{ minWidth: '700px' }} className="tab-content-inner min-width-desktop">
                      <div className="sticky-item tab-content-list mb-2">
                        ...
                        <div style={{ overflow: 'visible' }} className="tab-content-header">
                          {t("Статус")} <Hint customLeft="-1px" hintMessage={
                            <span>
                              <p>{t("Статусы сообщений")}:</p>
                              <span style={{
                                marginTop: '4px',
                                textAlign: 'left',
                                fontSize: '10px',
                                ...
                                вне зоны действия сети или аппарат абонента выключен.
                                 "Заблокировано" - сообщение заблокировано по финансовой причине или
                                 по желанию клиента.
                                 "Доставляется" - сообщение не получило окончательный статус (время жизни смс 24 часа,
                                 в течение этого периода статус обновится).`
                                }
                              </span>
                            </span>}/>
                        </div>
                      ...
                      {notification.history.map((historyItem, i) => {
                        ...
                        return (
                          <div
                            className="tab-content-list mb-2"
                            key={`email-page_tab-content-list-iten-${i}`}
            ...
            <div className="modal-content">
              ...
              <div className="form-group mr-3 ml-3">
                ...
                <div
                  ...
                  <button type="button"
                    className={'button' + (notification.balance && (receivers_email > notification.balance.emailAmount)
                      && ' disabledField')}
                    onClick={() => receivers_email <= (notification.balance && notification.balance.emailAmount)
                      && this.setEmail()}
                    data-dismiss="modal"
                  >
                    {t("Начать")}
            ...
            <div className="modal-content">
              ...
              <div className="form-group mr-3 ml-3">
                ...
                <div className="buttons d-flex flex-column justify-content-center align-items-center">
                  <button type="button"
                    className={'button' + (notification.balance && (count_sms_all > notification.balance.smsAmount) &&
                      ' disabledField')}
