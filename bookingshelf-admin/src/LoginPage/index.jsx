...
class Index extends React.Component {
    ...
    componentDidMount() {
        ...
        document.title = "Логин в системе Онлайн-запись";
    ...
    render() {
        ...
        return (
            <div>
                <div className="container-bg slideLeft"/>
                    ...
                    <div className="sign_in">
                        <div>
                            <form name="form" onSubmit={this.handleSubmit}>
                                ...
                                <input type="text" className={'form-group' + (submitted && !login ? ' redBorder' : '')} name="login" value={login} onChange={this.handleChange} />
                                 <span>Пароль</span>
                                    <input type="password" className={'form-group' + (submitted && !password ? ' redBorder' : '')} name="password" value={password} onChange={this.handleChange} />
                                {authentication && authentication.error && authentication.error.code === 5 &&
                                <p className="alert-danger p-1 rounded pl-3 mb-2">Аккаунт не активирован. Проверьте email и завершите регистрацию</p>
                                }
                                {authentication && authentication.error && authentication.error.code === 6 &&
                                <p className="alert-danger p-1 rounded pl-3 mb-2">Неверный логин или пароль</p>
                                }
                                        <button className="button text-center" type="submit">ВОЙТИ</button>
                                ...
                                <a href="#" onClick={()=>this.resetPassword()} className="popup_forgot" data-toggle="modal"
                                   data-target=".modal_reset_password">Забыли пароль?</a>
                            </form>
                        </div>
                        ...
                        <div className="modal-dialog modal-dialog-lg modal-dialog-centered">
                            <div className="modal-content">
                                ...
                                <div className="form-group mr-3 ml-3">
                                    ...
                                    <p className="alert-danger p-1 rounded pl-3 mb-2">Такого email не существует!</p>
                                    }
                                    {
                                        authentication && authentication.status && authentication.status === 'reset.email' && (!authentication.error || authentication.error===-1) &&
                                        <p className="alert-success p-1 rounded pl-3 mb-2">Инструкция по сбросу пароля отправлена на Ваш Email</p>
                                    }
                                    <button
                                        className={((!forgottenEmail || (!emailIsValid) || authentication.forgotPassLoading) && 'disabledField') + " button text-center"}
                                        type="button"
                                        onClick={forgottenEmail && emailIsValid && this.sendRequest}>Отправить
                                    </button>
    ...
    resetPassword () {
        this.setState({ ...this.state, resetPasswordModal: true });
    }
