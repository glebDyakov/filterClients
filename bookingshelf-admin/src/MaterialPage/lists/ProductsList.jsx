      ...
      <div className="tab-content-list mb-2">
        ...
        <div className="overflow-visible reserve-title">
          <p><span className="mob-title">{t('Остаток')} /<span className="red-text"> {t('Резерв')}</span>
                        ...
                        />: </span>
            {product.currentAmount} {t('шт')} ({product.currentNominalAmount} {activeUnit && this.getUnitName(activeUnit.unitName)}) {product.reserve > 0 ?
              <span
                className="red-text">/ {Math.round(product.reserve)} {activeUnit && this.getUnitName(activeUnit.unitName)}</span> : '/ -'}
