...
class PercentSettings extends Component {
  ...
  initData(context = this.context) {
    ...
    const productsCategories = material.categories.map((category) => {
      const categoryPercent = context.payroll.categoriesPercent.find((categoryPercent) => categoryPercent.categoryId === category.categoryId);
      const products = material.products.filter((product) => product.categoryId === category.categoryId);
      return {
        id: category.categoryId,
        title: category.categoryName,
        color: '',
        amount: categoryPercent ? categoryPercent.percent : '',
  ...
  getPrevItem(type, id) {
    switch (type) {
      ...
      case 'categories':
        return this.context.payroll.categoriesPercent.find((category) => category.categoryId === id);
      case 'products':
  ...
  handleSubmitGroup(item, type) {
    ...
    if (itemType === 'services') {
      ...
      this.context.handleUpdatePercents('categories', [this.getSubmitNestedItem(item)]);
    }
  }
  ...
  render() {
    ...
    return (
      <div className="percent-of-sales-container">
        ...
        {this.context.payroll.isSavingServiceGroupsPercent || this.context.payroll.isSavingServicesPercent || this.context.payroll.isSavingCategoriesPercent || this.context.payroll.isSavingProductsPercent &&
        <img className="d-block" style={{ width: '57px' }}
        ...
        <div className="percent-of-sales">
          {(this.context.payroll.servicesSaveStatus === 200
            ...
            || this.context.payroll.categoriesSaveStatus === 200
          ...
          <div className="services-container col">
            <p className="percent-title">{t('Услуги')}</p>
