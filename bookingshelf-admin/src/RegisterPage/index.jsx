...
class Index extends React.Component {
  ...
  componentWillReceiveProps(newProps) {
    if ( JSON.stringify(this.props.authentication) !== JSON.stringify(newProps.authentication)) {
      ...
      if (newProps.authentication && newProps.authentication.status &&
        newProps.authentication.status === 'register.company' &&
        (!newProps.authentication.error || newProps.authentication.error === -1)
      ) {
      ...
      if (newProps.authentication.status !== 'register.company' && newProps.authentication &&
        newProps.authentication.error && (newProps.authentication.error.code === 1 ||
          newProps.authentication.error.code === 3 || newProps.authentication.error.length > 1)
      ) {
        setTimeout(() => {
      ...
      if (authentication && authentication.status && authentication.status === 'register.company'
  ...
  handleSubmit(event) {
    ...
    if (emailIsValid && user.companyName && user.email && user.password && user.password === user.password_repeated &&
      user.timezoneId !== '' && user.countryCode !== '' && agreed && !authentication.registering
    ) {
    ...
    const modal = this.state.isOpenAuthModal &&
      <div className="message-is-sent-wrapper">
        <div className="message-is-sent-modal">
          <button onClick={()=> this.setState({ isOpenAuthModal: false })} className="close" />
          ...
          <div className="sign_up">
                  ...
                  <div className="col-md-6 mx-auto pr-md-0">
                    ...
                    <input
                      ...
                      className={'' + (user.password && !user.password_repeated ||
                        (user.password_repeated && user.password !== user.password_repeated) ? ' redBorder' : '')
                      }
            ...
            <div className="bottom-container">
              ...
              <button form="sign-up-form"
                className={((!emailIsValid || !user.companyName.replace(/[ ]/g, '') || user.countryCode === '' ||
                  (user.password !== user.password_repeated) || user.timezoneId === '' ||
                  user.password.replace(/[ ]/g, '') === '' || authentication.registering) ||
                !agreed ? 'disabledField' : '') + ' button sign_up_button'
                }
                type={emailIsValid && user.companyName.replace(/[ ]/g, '') && user.countryCode !== '' &&
