...
function toggleMoveVisit(isMoveVisit) {
  ...
  function success(isMoveVisit) {
    return { type: appointmentConstants.MOVE_VISIT_SUCCESS, payload: { isMoveVisit } };
  }
}
...
function toggleStartMovingVisit(isStartMovingVisit) {
  ...
  function success(isStartMovingVisit) {
    return { type: appointmentConstants.START_MOVING_VISIT_SUCCESS, payload: { isStartMovingVisit } };
  }
}
...
function makeMovingVisitQuery(data) {
  return (dispatch) => {
    const {
      appointments, staff, reservedTimes, timetable, movingVisit,
      movingVisitDuration, movingVisitStaffId, movingVisitMillis, prevVisitStaffId,
    } = data;
    let shouldMove = true;
    ...
    const prevVisualVisit = JSON.parse(JSON.stringify(movingVisit));
    const coStaffs = movingVisit.coStaffs;

    if (shouldMove) {
      const movingVisitEndTime = movingVisitMillis + (movingVisitDuration * 1000);

      const timetableItems = timetable
        .filter((item) => item.staffId === movingVisitStaffId ||
          (movingVisit.coStaffs && movingVisit.coStaffs.some((coStaff) => coStaff.staffId === item.staffId)),
        );

      shouldMove = timetableItems.every((timetableItem) => {
        const checkOnMovingVisit = (i) => (
          (prevVisitStaffId === movingVisitStaffId ||
            (movingVisit.coStaffs && movingVisit.coStaffs
              .some((coStaff) => coStaff.staffId === timetableItem.staffId)
            )
          ) && movingVisit.appointmentTimeMillis <= i &&
          (movingVisit.appointmentTimeMillis + (movingVisitDuration * 1000)) > i
        );
    ...
    if (shouldMove) {
      ...
      dispatch(calendarActions.makeVisualMove(
        { ...prevVisualVisit, staffId: prevVisitStaffId },
        movingVisitStaffId,
        movingVisitMillis,
        coStaffs,
      ));
