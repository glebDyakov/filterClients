...
class AddAppointment extends React.Component {
    ...
    componentWillReceiveProps(newProps) {
        ...
        if ( JSON.stringify(this.props) !==  JSON.stringify(newProps)
            ...
            this.setState({
                ...
                appointment: {appointmentTimeMillis:newProps.clickedTime!==0?newProps.clickedTime:'',
                    duration: newProps.appointmentEdited?newProps.appointmentEdited[0][0].duration:'',
                    description: newProps.appointmentEdited?newProps.appointmentEdited[0][0].description:'',
                    customId: newProps.appointmentEdited?newProps.appointmentEdited[0][0].customId:'' },
    ...
    getInfo(appointment){
        ...
        let service=services.servicesList && services.servicesList.filter((service, i) => service.serviceId===appointment.serviceId);

        this.setState({serviceCurrent: {...serviceCurrent, id:appointment.serviceId, service:service[0] }, clientChecked:client})
    }

    setTime(appointmentTimeMillis, minutes){
        const {appointment, timeNow}=this.state
        ...
        this.setState({
            appointment: {
                ...appointment,
                appointmentTimeMillis: startTime
            }
        })
        this.setState({
            timeArrange:timing,
    ...
    render() {
        ...
        let servicesDisabling=services.servicesList && services.servicesList.some((service)=>parseInt(service.duration)/60<=parseInt(timeArrange));

        console.log(this.state)

        return (
            <div className="modal fade new_appointment">
                        ...
                        <div className="form-group p-4">
                            <div className="row">
                                <div className="calendar col-lg-6">
                                    ...
                                    <p className="title">Запись</p>
                                    <div className="row">
                                        {minutes.length!==0&&<div className="col-md-4">
                                            <p>Начало</p>
                                            <TimePicker
                                                value={appointment.appointmentTimeMillis&&moment(appointment.appointmentTimeMillis, 'x') }
                                                ...
                                                onChange={(appointmentTimeMillis)=>this.setTime(appointmentTimeMillis, minutes)}
                                            />
                                        </div>}
                                    ...
                                    <div className="row">
                                        <div className="col-md-4">
                                            ...
                                            <input className={!servicesDisabling&&'disabledField'} type="text" disabled="disabled" placeholder=""  name="duration" value={serviceCurrent.service && serviceCurrent.service.length!==0 ? moment.duration(parseInt(serviceCurrent.service.duration), "seconds").format("h[ ч] m[ мин]") : ''}/>
                                        </div>
                                        <div className="col-md-8">
                                            <p>Сотрудник</p>
                                    ...
                                    <textarea className="mb-3" placeholder="Например: Без окраски" name="description"  value={appointment.description} onChange={this.handleChange}/>
                                    {
                                        appointments.status === 200 &&
                                        <p className="alert-success p-1 rounded pl-3 mb-2">Запись сохранена</p>
                                    }
                                    <button className={(appointments.status === 208 && !staffCurrent.staffId || !clientChecked.clientId || !appointment || !appointment.appointmentTimeMillis || serviceCurrent.service.length==0) ? 'button text-center button-absolute disabledField': 'button text-center button-absolute'} type="button"
                                            onClick={edit_appointment?this.editAppointment:this.addAppointment} disabled={appointments.status === 208 || serviceCurrent.service.length==0 || !staffCurrent.staffId || !clientChecked.clientId || !appointment || !appointment.appointmentTimeMillis}>Сохранить
                                    </button>
                                    {appointments.adding &&
                                    ...
                                    <div className="calendar">
                                        ...
                                        {!clientChecked || clientChecked.length==0 &&
                                            <div className="list-block-right mb-3">
                                                ...
                                                <div className="row">
                                                    <div className="col-sm-12"><p className="title mb-3">Добавить
                                                        клиента</p></div>
                                                </div>
    ...
    checkUser(client){
        this.setState({...this.state, clientChecked: client});
    }

    removeCheckedUser(){
        this.setState({...this.state, clientChecked: []});
    }
    ...
    addAppointment (){
        ...
        this.setState({...this.state, serviceCurrent: {
                id:-1,
                service:[]
            }, clientChecked: [] });
    ...
    editAppointment (){
        ...
        return editAppointment({...appointment, appointmentId:editedElement&&editedElement[0][0].appointmentId, serviceId:serviceCurrent.id, staffId:staffId.staffId, clientId:clientChecked.clientId, approved: true})
    }
    ...
    setService(serviceId, service) {
        ...
        this.setState({...this.state, serviceCurrent: {...serviceCurrent, id:serviceId, service:service }, appointment: {...appointment, duration: service.duration }});
    }
