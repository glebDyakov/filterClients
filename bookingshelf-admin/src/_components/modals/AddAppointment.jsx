...
class AddAppointment extends React.Component {
  ...
  componentWillReceiveProps(newProps) {
    ...
    if (this.state.shouldUpdateCheckedUser && JSON.stringify(this.props.clients) !== JSON.stringify(newProps.clients)) {
      const user = newProps.clients.client && newProps.clients.client.length > 0 && newProps.clients.client.find((cl) => cl.phone === newProps.checkedUser.phone);
      let finalUser = {};
  ...
  getOptionList(index) {
    ...
    if (activeStaffTimetable) {
      finalOptionList = optionList.filter((option) => {
        ...
        const lastAppointmentIndex = appointment.length - 1;
        const lastAppointmentEndTime = appointment[lastAppointmentIndex].appointmentTimeMillis + (appointment[lastAppointmentIndex].duration * 1000);
  ...
  setTime(appointmentTimeMillis, minutes, index) {
    ...
    const startTime = parseInt(moment(moment(timeNow, 'x').format('DD/MM/YYYY') + ' ' + moment(appointmentTimeMillis).format('HH:mm'), 'DD/MM/YYYY HH:mm').format('x'));
    const timing = this.getTimeArrange(parseInt(moment(appointmentTimeMillis).format('x')), minutes, appointment);
  ...
  getServiceList(index) {
    ...
    return <p
      className="staffAlert-noService">{this.props.t('Нет доступных услуг. Выберите сотрудника в настройках услуг')}</p>;
  }
        ...
        <div style={{ paddingTop: '4px', display: 'flex', padding: 0 }} className="col-sm-12 mt-2">
          ...
          <span style={{ width: 'auto', margin: '0' }} className="justify-content-end check-box">
            <label>
              ...
              />
              <span style={{ margin: '0 0 0 4px' }} data-label-off={this.props.t('Выкл')}
                    data-label-on={this.props.t('Вкл')} className="check"/>
            </label>
          </span>
        </div>
  ...
  renderMenuItemChildren(option, props) {
    ...
    const visibleKeys = ['firstName', 'lastName', 'phone', 'email', 'carBrand', 'carNumber'];
            ...
            <div className="modal-content">
              <div className="modal-header">
                {edit_appointment ? <h4 className="modal-title">{t('Редактировать запись')}</h4> :
                  <h4
                    className="modal-title">{t('Новая запись')}, {moment(timeNow, 'x').locale(t('locale')).format('dddd DD MMMM, YYYY')}</h4>}
              ...
              <div className="form-group form-group-addService">
                <div className="row">
                  <div className="calendar col-xl-8 pr-0">
                    <div className="appointments-container">
                      {appointment.map((appointmentItem, index) => {
                        return <div className="addApointmentEachBlock">
                          ...
                          {appointment.length > 1 && <button className="close removeService-button"
                            ...
                            {index === 0 && (
                              <div className="appointment-staff add_appointment_staff">
                                ...
                                <div className="dropdown add-staff">
                                  <a
                                    className={edit_appointment || timeArrange === 0 ? 'disabledField dropdown-toggle drop_menu_personal' : 'dropdown-toggle drop_menu_personal'}
                    ...
                    {appointmentMessage &&
                    <div style={{ margin: '-20px 0 32px 0', padding: '4px 12px' }}
                         className="desktop-visible alert alert-danger">{appointmentMessage}</div>
                    }
                    {adding &&
                      ...
                      <div className="addApointmentClientBlock">
                        ...
                        <div className="row">
                          <div className="col-sm-12">
                            <p
                              className="add-helpers-text mobile-visible">{companyTypeId === 4 ? t('Добавление пациента') : t('Добавление клиента')}
                              <Hint
                                hintMessage={t('Начните поиск. Если клиента нет в базе, заполните необходимые поля')}/>
                            </p>
                            <p
                              className="add-helpers-text mb-3 desktop-visible">{companyTypeId === 4 ? t('Добавление пациента') : t('Добавление клиента')}
                              <Hint
                                hintMessage={t('Начните поиск. Если клиента нет в базе, заполните необходимые поля')}/>
                            </p>
                          </div>
                          ...
                          <div className="col-12 py-4 py-mb-0">
                            {Object.entries({
                              ...
                              clientPhone: typeAheadOptions.clientPhone,
                                ...
                                <div key={key}
                                     className={'typeahead-wrapper' + (value.isValid(this.state[key]) ? '' : ' redBorderWrapper')}>
                        ...
                        <p className="appointments-all-price">
                          {t('Общая сумма')}: {appointmentsAllPrice - appointmentsAllPrice * (this.state.clientChecked ? this.state.clientChecked.discountPercent : 0) / 100} {this.state.serviceCurrent && this.state.serviceCurrent.length > 0 ? this.state.serviceCurrent[0].service.currency : ''}
                        </p>
  ...
  getAppointments(appointment, deletedFirstVisit) {
    const { appointmentsFromProps, reservedTimeFromProps, staff } = this.props;
    ...
    appointment.forEach((item, i) => {
      ...
      if (i !== 0) {
        ...
        console.log('appointmentsFromProps', appointmentsFromProps);
      }
