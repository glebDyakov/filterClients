...
class AddBlackList extends React.Component {
    constructor(props) {
        ...
        this.addUsersToBlackList = this.addUsersToBlackList.bind(this);
        this.addUserToBlackListedArray = this.addUserToBlackListedArray.bind(this);
    }

    addUsersToBlackList() {
        if (this.state.arrayUsersToBlackList && this.state.arrayUsersToBlackList.length > 0) {
            this.state.arrayUsersToBlackList.map(item => {
    ...
    addUserToBlackListedArray(item) {
        if (this.state.arrayUsersToBlackList.includes(item)) {
            this.setState({
                arrayUsersToBlackList: this.state.arrayUsersToBlackList.filter(el => el !== item)
            });
        } else {
            this.setState({
                arrayUsersToBlackList: [...this.state.arrayUsersToBlackList, item]
            })
        }
    }
    ...
    render() {
        ...
        return (
            <Modal size="md" onClose={this.closeModal} showCloseButton={false} className="mod black-list-modal-wrapper">
                <div className="modal-add-black-list">
                    {client &&
                    <div>
                                    ...
                                    <ul>
                                        {clients.client && clients.client.map((client_user, i) =>
                                            <BlackListUserItem
                                                ...
                                                handleAddUser={this.addUserToBlackListedArray}
                                                isChecked={this.state.arrayUsersToBlackList.includes(client_user)}
                                            />
                                        )}
                                    </ul>
    ...
    updateClient(activeClient) {
        ...
        return updateClient(activeClient, true);
