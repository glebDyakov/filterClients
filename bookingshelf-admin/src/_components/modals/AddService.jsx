...
class AddService extends React.Component {
    ...
    this.state = {
      service: (props.group_working && props.editServiceItem)
        ? {
          ...
          ...props.group_working,
          usingMaterials: !!(props.group_working.serviceProducts && props.group_working.serviceProducts.length),
  ...
  componentWillReceiveProps(newProps) {
    ...
    if (JSON.stringify(this.props.services.serviceProducts) !== JSON.stringify(newProps.services.serviceProducts)) {
      ...
      this.setState({
        service: {
          ...this.state.service,
          usingMaterials: !!serviceProducts.length,
          serviceProducts: serviceProducts.length ? serviceProducts : [{
            'amount': '',
            'productId': '',
          }],
        },
  ...
  getOptionList() {
    ...
    for (let i = booktimeStep; i <= 54000; i += booktimeStep) {
      ...
      const minute = i % 3600;
      options.push(<option
        value={i}>{`${hour ? `${hour} ч ` : ''}${minute ? `${minute / 60} ${t('минут')}` : `00 ${t('минут')}`}`}</option>);
    }
    return options;
  }
            ...
            <div className="modal-content">
              ...
              <div className="modal-body">
                <div className="row d-flex justify-content-between">
                  <div className="left-container">
                    ...
                    <div className="input_limited_wrapper_name">
                      ...
                      <input style={{ paddingRight: '60px' }} type="text" placeholder="" name="name" maxLength="128"
                             value={service.name} onChange={this.handleChange}
                             className={((service.priceFrom || service.specialPrice) && service.name === '' ? ' redBorder' : '')}/>
                      ...
                      <div className="row col-md-6">
                        <div className="price-from col-md-6">
                          ...
                          <input type="number" placeholder="" name="priceFrom"
                                 value={service.priceFrom} onChange={this.handleChangePrice}
                                 className={(service.specialPrice && service.priceFrom === '' ? ' redBorder' : '')}/>
                        </div>

                        <div className="price-to col-md-6">
                          <p>{t('Цена до')}</p>
                          <input type="number" placeholder="" name="priceTo"
                                 value={service.priceTo} onChange={this.handleChange}
                                 className={(service.priceFrom && service.priceFrom !== 0 && service.priceTo !== '' && service.priceTo !== 0 && parseInt(service.priceTo) < parseInt(service.priceFrom) ? ' redBorder' : '')}/>
                        ...
                        .map((item, index) => {
                          return <div className="select-color using-materials-dropdown dropdown mb-3 border-color">

                            {
                              ...
                              <a onClick={() => {
                                ...
                                <span
                                  ...
                                  className="items-color"><span></span>    <span></span>  <span></span></span></span>
                              </a>}
                              ...
                              <li className="search-item">
                                <div className="align-items-center clients">
                                  <div className="search">
                                    <input type="search"
                                           placeholder={t('Введите название товара') + ', ' + t('Код товара').toLowerCase()}
                                           aria-label="Search"
                              ...
                              <li className="services_list_wrapper">
                                ...
                                <ul className="pt-0">
                                  ...
                                  {this.props.material.products.filter((product) => service.serviceProducts
                                    ...
                                    return (<li onClick={() => this.handleChooseProduct(product, index)}
                                      ...
                                      <p className="product-info">
                                        <span onClick={() => {
                                          console.log(product);
                                              ...
                                              className="product-amount">{t('На складе')}: {product && product.currentAmount} {t('шт')} / ({product && product.currentNominalAmount} {product && this.getActiveUnitName(product)})</span>
                            ...
                            <div className="select-material row col-12 col-md-4">
                              <InputCounter placeholder={`${t('Например')}, 100 мл`}
                                            value={String(this.state.service.serviceProducts[index].amount)}
                                            title={`${t('Норма списания')}, ${item.unitName ? (item.unitName) : ''}`}
                                            name="amount" handleChange={(e) => this.handleChangeProduct(e, index)}
                                            maxLength={9}/>
                            </div>
                            {/* {index !== 0 && <button className="close"   onClick={()=>this.removeMaterial(index)}>x</button>}*/}
                            {this.state.service.serviceProducts.length !== 1 &&
                            <button className="close" onClick={() => this.removeMaterial(index)}>x</button>}
                          </div>;
                        })}
                      ...
                      <div className="header-container">
                        ...
                        <div className="search dropdown row">
                          <form className="col-sm-12 form-inline" data-toggle="dropdown">
                            <input type="search" placeholder={t('Поиск по имени')} aria-label="Search"
                                   ref={(input) => this.search = input} onChange={this.handleSearch}/>
                  ...
                  {message &&
                  <div style={{ padding: '4px 12px', margin: '16px', width: 'calc(100% - 42px)' }}
                       className="alert alert-danger">{message}</div>
                  }

                  <div
                    className="buttons col-12 flex-column flex-md-row d-flex justify-content-between align-items-center">
                    ...
                    <button
                      className={'button' + (!(!service.usingMaterials || ((service.serviceProducts.length) && (service.serviceProducts.every((item) => (item.productId && item.amount))))) ? ' disabledField' : '')}
                      type="button"
  ...
  toggleChangeStaff(staff) {
    ...
    if (!staffs) {
      ...
      const deleteFunc = staffs.map(function(o) {
        return o.staffId;
      }).indexOf(staff);
