...
class ExpenditureProduct extends React.Component {
  constructor(props) {
    super(props);
    let client;
    ...
    this.expenditureProduct = this.expenditureProduct.bind(this);
  }
  ...
  render() {
    const { company, client_working, material, t } = this.props;
    ...
    return (
      <div>
        <Modal size="xs" onClose={this.closeModal} showCloseButton={false} className="mod modal-remove-products">
          <div className="">
            {client &&
                        <div>
                            ...
                            <div className="form-group">
                              ...
                              <div className="row">
                                <div className="col-sm-12">
                                  ...
                                  <p>{t("Номинальный объем")}: <strong>{activeProduct && activeProduct.nominalAmount} {activeUnit && activeUnit.unitName}</strong></p>
                                  <p>{t("Остаток на складе")}: <strong> {activeProduct && activeProduct.currentAmount} </strong></p>
                                </div>
                              </div>

                              <div className="row">
                                <div className="col-sm-12">
                                  <InputCounter withCounter={true} title={`${t("Количество товара на списание")}${((client.nominalCheck === 'nominal') && activeUnit) ? `, ${activeUnit.unitName}` : ''}`} value={String(client.amount ? client.amount : '')}
                              ...
                              <select className="custom-select" name="target" onChange={this.handleChange}
                                value={client.target}
                              >
                                <option value="">{t("Выберите причину")}</option>
                                <option value="S">{t("Продажа")}</option>
                                <option value="I">{t("Внутренняя ошибка")}</option>
                                <option value="D">{t("Товар поврежден")}</option>
                                <option value="C">{t("Изменения наличия")}</option>
                                <option value="L">{t("Утеря")}</option>
                                <option value="O">{t("Другое")}</option>
                              </select>
                              ...
                              {invalidCount &&
                                    <p className="alert-danger p-1 rounded pl-3 mb-2">{t("Недостаточно товаров на складе")}</p>
                              }
  ...
  updateClient() {
    ...
    this.props.dispatch(materialActions.toggleUnit(client, true));
  };
  ...
  addClient() {
    ...
    this.props.dispatch(materialActions.toggleUnit(client));
  };
...
const connectedApp = connect(mapStateToProps)(withTranslation("common")(ExpenditureProduct));
export { connectedApp as ExpenditureProduct };
