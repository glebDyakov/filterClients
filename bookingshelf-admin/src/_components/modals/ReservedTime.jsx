...
class ReservedTime extends React.Component {
    ...
    componentWillReceiveProps(newProps) {
        if (JSON.stringify(this.props.staffs) !== JSON.stringify(newProps.staffs)) {
            this.setState({
                staffs: newProps.staffs
            })
        }
        if (JSON.stringify(this.props.minutesReservedtime) !== JSON.stringify(newProps.minutesReservedtime)) {
            this.setState({
        ...
        if (JSON.stringify(this.props.clickedTime) !== JSON.stringify(newProps.clickedTime)) {
            this.setState({
                timeNow: newProps.clickedTime === 0 ? moment().format('x') : newProps.clickedTime
            })
        }
        if (JSON.stringify(this.props.reservedStuffId) !== JSON.stringify(newProps.reservedStuffId)) {
            this.setState({
    ...
    render() {
        ...
        return (
            <Modal size="md" style={{maxWidth: '30%'}} onClose={this.closeModal} showCloseButton={false}
                   className="mod">
                        ...
                        <div className="modal-content reserved-time-modal">
                            ...
                            <div className="modal-body">
                                ...
                                <div className="dropdown add-staff mb-3">
                                    <a className={edit_reservedTime ? 'disabledField dropdown-toggle drop_menu_personal' : 'dropdown-toggle drop_menu_personal'}
                                       data-toggle={!edit_reservedTime && 'dropdown'}
                                    ...
                                    <ul className="dropdown-menu" role="menu">
                                        {
                                            timetable && timetable.map((staff, key) => {
                                                    ...
                                                    return (
                                                        <li onClick={() => this.setStaff(staff, staff.firstName, staff.lastName, staff.imageBase64)}
                                                            key={key}>
                                                            <a>
                                                                <div className="img-container">
                                                                    <img className="rounded-circle"
                                ...
                                />
                                <p>{t("Описание")}</p>
                                <input className="description-input" type="email" placeholder={t("Введите описание")}
                                       name="description" value={reservedTime.description} onChange={this.handleChange}
    ...
    setTime(reservedTimeSelected, field) {
        ...
        this.setState({
            reservedTime: {
                ...reservedTime, ...reservedTimeProps,
                [field]: reservedTimeSelected ? moment(reservedTimeSelected).format('x') : ''
            }
        });
    }
    ...
    setStaff(staffId, firstName, lastName, imageBase64) {
        this.setState({
            ...this.state, minutesReservedtime: this.getHours(staffId),
            staffCurrent: {
                staffId: staffId.staffId, firstName: firstName, lastName: lastName, imageBase64: imageBase64,
                reservedTime: {
                    startTimeMillis: '',
                    endTimeMills: '',
                    description: ''
