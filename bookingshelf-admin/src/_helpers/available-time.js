...
export const checkIsOnAnotherReservedTime = (staffWithReservedTimes, interval) => {
  return staffWithReservedTimes && staffWithReservedTimes.reservedTimes &&
    staffWithReservedTimes.reservedTimes.some((element) =>
...
export const checkIsOnAnotherVisit = (staffWithAppointments, interval) => {
  return staffWithAppointments && staffWithAppointments.appointments &&
    staffWithAppointments.appointments.some((element) =>
      _checkTimeForCalendarElement(
        element.appointmentTimeMillis,
        (element.appointmentTimeMillis + (element.duration * 1000)),
        interval,
      ),
    );
};
...
export const getStaffListWithAppointments = ({ appointments, staffWithTimetable, staff, excludeCurrentStaff }) => {
  ...
  return appointments
    ? appointments.filter((item) => staffAll
      .some((currentStaff) => (item.staff && item.staff.staffId) === currentStaff.staffId))
    : [];
};

export const isAvailableTime = (
  startTime, endTime, staffWithTimetable, appointments, reservedTimes, staff, extraCheck = null,
) => {
  const intervals = [];
  ...
  const staffWithReservedTimes = reservedTimes && reservedTimes
    .find((item) => (item.staff && item.staff.staffId) === staffWithTimetable.staffId);
  ...
  return intervals.every((interval) => {
    const isOnAnotherVisit = staffListWithAppointments && staffListWithAppointments
      .some((staffWithAppointments) => checkIsOnAnotherVisit(staffWithAppointments, interval));
    ...
    return staffWithTimetable && staffWithTimetable.timetables && staffWithTimetable.timetables.some((time) => {
      ...
      return (isIncludedInTimetable && !isOnAnotherVisit && !isOnAnotherReservedTime) ||
        (extraCheck && extraCheck(interval));
    });
  });
};

export const getNearestAvailableTime = (
  startTime, endTime, timetableItems, appointments, reservedTimes, staff, extraCheck,
) => {
  let availableCellTime = endTime;
  endTime = endTime + (15 * 60000);
  const isNextCellAvailable = timetableItems.every((staffWithTimetable) =>
    isAvailableTime(startTime, endTime, staffWithTimetable, appointments, reservedTimes, staff, extraCheck));
  if (isNextCellAvailable) {
    availableCellTime = getNearestAvailableTime(
      startTime, endTime, timetableItems, appointments, reservedTimes, staff, extraCheck,
    );
  }
