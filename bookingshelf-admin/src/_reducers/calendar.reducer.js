...
export function calendar(state = initialState, action) {
  switch (action.type) {
    ...
    case calendarConstants.ADD_APPOINTMENT_SUCCESS:
      ...
      //         action.appointment
      //         .filter(item => app.staff.staffId === action.staffId && app['appointments'].push(item))
      //     );
      //
      //     if (boolAppointments.length !== 0) {
      //     } else {
      //         action.appointment
      //         .forEach(item => appointments.push({'staff': {'staffId': action.staffId}, 'appointments': [item]}))
      //     }
      // } else {
      //     appointments = action.appointment
      //     .map(item => ({'staff': {'staffId': action.staffId}, 'appointments': [item]}))
      // }
      return {
        ...state,
        status: 200,
    ...
    case calendarConstants.ADD_RESERVED_TIME_SUCCESS:
      ...
      const reservedTimes = state.reservedTime;
    ...
    case calendarConstants.EDIT_APPOINTMENT_SUCCESS:
      ...
      const appointmentsLoaded = state.appointments;
    ...
    case calendarConstants.APPROVE_APPOINTMENT_SUCCESS:
      ...
      const appointmentsApproved = state.appointments;
    ...
    case calendarConstants.CANCEL_VISUAL_DELETING:
      ...
      const updatedAppointmentsCanceled = [];
      newAppointment.forEach((localAppointment) => {
    ...
    case calendarConstants.DELETE_APPOINTMENT_NEW_SOCKET:
      ...
      newAppointment = state.appointments;
      const appointmentsToDelete = action.payload.payload;
    ...
    case calendarConstants.MOVE_APPOINTMENT_NEW_SOCKET:
      newAppointment = state.appointments;
      newAppointmentsCount = state.appointmentsCount;
      const appointmentsToMove = action.payload.payload;

      appointmentsToMove.forEach((newItem) => {
        if (newAppointment && newAppointment.length) {
          newAppointment.forEach((item) => {
            const appointmentIndex = item.appointments && item.appointments
              .findIndex((item) => newItem.appointmentId === item.appointmentId);
