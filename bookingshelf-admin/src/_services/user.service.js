...
function register(user) {
  ...
  return fetch(`${origin}${config.apiUrl}/signup`, requestOptions)
    .then((data) => handleResponse(data, requestOptions)).then((user) => {
    // login successful if there's a jwt token in the response
    // store user details and jwt token in local storage to keep user logged in between page refreshes
    // localStorage.setItem('user', JSON.stringify(user));
      return user;
...
function forgotPass(email) {
  ...
  return fetch(`${origin}${config.apiUrl}/resetpassword`, requestOptions)
    .then((data) => handleResponse(data, requestOptions)).then((email) => {
      return email;
    });
}
...
function update(user) {
  ...
  return fetch(`${origin}${config.apiUrl}/users/${user.id}`, requestOptions)
    .then((data) => handleResponse(data, requestOptions)); ;
}
...
function updateProfile(user) {
  ...
  return fetch(`${origin}${config.apiUrl}/profile`, requestOptions)
    .then((data) => handleResponse(data, requestOptions));
}
